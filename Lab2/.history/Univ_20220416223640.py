...
with open("univ.txt", "w", encoding="UTF=8") as file:
    ...
    for d in cursor.execute(
                "SELECT id FROM staff WHERE name=? AND url=?",
                    [staff["name"], staff["url"]]
                ):
                    staff["id"] = d[0]

    if not staff.get("id"):
                    cursor.execute(
                        "INSERT INTO staff (name, url, faculty_id) VALUES (?,?,?)",
                        [staff["name"], staff["url"], faculty["id"]]
                )
    connection.commit()
    for d in cursor.execute(
                    "SELECT id FROM staff WHERE name=? AND url=?",
                    ...
                    staff["id"] = d[0]
    faculty["staff"].append(staff)
    staff.append(faculty)
